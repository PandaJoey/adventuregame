...
{
	//returns how big the box is
	FVector Extent = SpawningBox->GetScaledBoxExtent();
	//returns origin
	FVector Origin = SpawningBox->GetComponentLocation();
	//finds random point between the origin and extent of the box to spawn a mob
	...
	if(ToSpawn)
	{
		...
		if(World)
		{
			...
			if(Enemy)
			{
				...
				AAIController* AIControl = Cast<AAIController>(Enemy->GetController());
				if (AIControl)
				{
					Enemy->AIController = AIControl;
				}
